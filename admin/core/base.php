...
function load_view_diff_mod($name, $data_send = array(), $module = null) {
    global $data;
    $data = $data_send;

    // Nếu không chỉ rõ module, sử dụng module hiện tại
    if (!$module) {
        $module = get_module(); // Lấy module hiện tại
    }

    // Đường dẫn đến view của module cụ thể
    $path = MODULESPATH . DIRECTORY_SEPARATOR . $module . DIRECTORY_SEPARATOR . 'views' . DIRECTORY_SEPARATOR . $name . 'View.php';
    ...
    if (file_exists($path)) {
        ...
        // Nếu không tìm thấy view, thông báo lỗi
        echo "Không tìm thấy {$path}";
    }
}
